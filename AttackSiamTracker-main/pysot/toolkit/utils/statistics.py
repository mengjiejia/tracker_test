...
def success_overlap(gt_bb, result_bb, n_frame):
    ...
    for i in range(len(thresholds_overlap)):
        # get the ratio of the number of frames of which iou greater than the threshold
        success[i] = np.sum(iou > thresholds_overlap[i]) / float(n_frame)
    # success is a 1D array with length of 20
    return success


def my_success_overlap(gt_bb, result_bb, n_frame):
    ...
    for i in range(n_frame):
        ...
        #    success[i] = iou[i]
        success[i] = iou[i]
    # success is a 1D array 1D array with length of n_frame. Element value equals to 0 if iou less than the threshold, otherwise iou
    return success
...
def my_success_error(gt_center, result_center, n_frame):
    # n_frame = len(gt_center)
    success = np.zeros(n_frame)
    dist = np.ones(len(gt_center)) * (-1)
    mask = np.sum(gt_center > 0, axis=1) == 2
