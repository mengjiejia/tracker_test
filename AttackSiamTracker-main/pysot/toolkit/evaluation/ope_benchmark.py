...
from pysot.toolkit.utils.statistics import success_overlap, success_error, my_success_overlap, my_success_error
...
class OPEBenchmark:
    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # ad2attack evaluation
                success_ret_[video.name] = success_overlap(gt_traj, tracker_traj, n_frame)
                # get tracking successful frames
                # threshold = 0.6
                #success_ret_[video.name] = my_success_overlap(gt_traj, tracker_traj, n_frame)
                # print(video.name)
    ...
    def eval_precision(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # get tracking successful frames
                # threshold = 20
                #precision_ret_[video.name] = my_success_error(gt_center, tracker_center, n_frame)
    ...
    def show_result(self, success_ret, precision_ret=None,
                    norm_precision_ret=None, show_video_level=False, helight_threshold=0):
        """pretty print result
        Args:
        ...
        for tracker_name in tracker_names:
            ...
            if precision_ret is not None:
                ...
                #print(np.mean(list(precision_ret[tracker_name].values()), axis=0).shape)
        ...
        if show_video_level and len(success_ret) < 10 \
            ...
            print("\n\n")
            print('Performance for each video')
            ...
            for video in videos:
                row = "|{:^21}|".format(video)
                for tracker_name in success_ret.keys():
                    # print('success_ret[tracker_name][video]' ,len(success_ret[tracker_name][video]))
                    # print('precision_ret[tracker_name][video]', len(precision_ret[tracker_name][video]))
